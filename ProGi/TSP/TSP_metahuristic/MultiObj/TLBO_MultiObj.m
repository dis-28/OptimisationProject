...
function [bestsol,BestFitIter,bestfitness,P,f]=TLBO_MultiObj(Np,T,lb,ub,prob,maxProfit)
[numCities,~,~,~,~,~]=TSPdata;
...
for p = 1:Np
    [f(p),P(p,:)] = prob(P(p,:),maxProfit);            % Evaluating the fitness function of the initial population
end
BestFitIter(1) = min(f);
...
for t = 1: T
    ...
    for i = 1:Np
        ...
        [fnew,Xnew] = prob(Xnew,maxProfit);          % Evaluating the fitness of the newly generated solution
