scalar
   n number of cities  /29/
   m number of salesmen  /1/
   p maximum number of cities a salesmen can visit  /29/
   cpd cost incurred per KM distance it travelled  /2/
;
...
eq1..  sum(i,sum(j,c(i, j) * x(i, j)*cpd ))=e=F;  
eq2(j)$(ord(j) >= 2).. sum(i,x(i,j))=e=1;
eq3(i)$(ord(i) >= 2).. sum(j,x(i,j))=e=1;
eq4(i,j)$(ord(i) >= 2$(ord(i) >= 2)$(ord(i) <> ord(j))).. u(i)-u(j)+(n-m)*x(i,j) =l=n-m-1;

start.. sum(j, x('1',j)$(ord(j)>=2)) =e= m;
end.. sum(i, x(i,'1')$(ord(i)>=2)) =e= m;
